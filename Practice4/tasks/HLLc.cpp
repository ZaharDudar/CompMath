...
/**
 * @brief Оценка значений Sl и Sr по Эйнфилдту
 *
 * @cite 328 страница Toro E. F.
 */
waveSpeed EinfieldtSpeed(const Vec3& left, const Vec3& right)
{
    double rhoL = left[0];
    double uL = left[1]/rhoL;
    double eL = left[2];
    double pL = (gm-1)*(eL - 0.5 * rhoL * uL*uL);
    // double pL = (gm-1)*rhoL*eL;
    double aL = sqrt(gm * pL / rhoL);

    double rhoR = right[0];
    double uR = right[1]/rhoR;
    double eR = right[2];
    double pR = (gm-1)*(eR - 0.5 * rhoR * uR*uR);
    // double pR = (gm-1)*rhoR*eR;
    double aR = sqrt(gm * pR / rhoR);

    double eta = 0.5 * sqrt(rhoL * rhoR) / pow(sqrt(rhoL) + sqrt(rhoR),2);
    double d2 = (sqrt(rhoL) * aL * aL + sqrt(rhoR) * aR * aR) / (sqrt(rhoL) + sqrt(rhoR)) + eta * pow(uR - uL,2);
    double u_ = (sqrt(rhoL) * uL + sqrt(rhoR) * uR) / (sqrt(rhoL) + sqrt(rhoR));
    return {u_ - sqrt(d2), u_ + sqrt(d2)};
};
...
Flux updateFlux(const U& u, const double time)
{
    ...
    for (int i=0; i<NX-1; i++)
    {
        ...
        auto waveSpeeds = EinfieldtSpeed(u[i], u[i+1]);
        double Sl = waveSpeeds.Sl;
        double Sr = waveSpeeds.Sr;
        double Sc = pR - pL + rhoL*uL*(Sl-uL) - rhoR*uR*(Sr-uR); // Скорость контактного разрыва
        Sc /= rhoL*(Sl - uL) - rhoR*(Sr - uR);

        Vec3 UcL = rhoL * (Sl - uL)/(Sl - Sc) * Vec3{1, Sc, eL/rhoL + (Sc - uL)*(Sc + pL/(rhoL*(Sl - uL)))};
        Vec3 UcR = rhoR * (Sr - uR)/(Sr - Sc) * Vec3{1, Sc, eR/rhoR + (Sc - uR)*(Sc + pR/(rhoR*(Sr - uR)))};

        Vec3 Fl = (u[i] + Vec3{0, 0, pL}) * uL + Vec3{0, pL, 0};
        Vec3 Fr = (u[i] + Vec3{0, 0, pR}) * uR + Vec3{0, pR, 0};

        Vec3 FcL = Fl + Sl * (UcL - u[i]);
        Vec3 FcR = Fr + Sr * (UcR - u[i+1]);
...
int main()
{
    ...
    std::ofstream p_f;
    std::ofstream t_f;
    rho_f.open(OUTPUT_DIR"RIMAN_HLLc_1_rho.csv");
    u_f.open(OUTPUT_DIR"RIMAN_HLLc_1_u.csv");
    e_f.open(OUTPUT_DIR"RIMAN_HLLc_1_e.csv");
    p_f.open(OUTPUT_DIR"RIMAN_HLLc_1_p.csv");
    t_f.open(OUTPUT_DIR"RIMAN_HLLc_1_t.csv");
    while (t<maxT)
    {
